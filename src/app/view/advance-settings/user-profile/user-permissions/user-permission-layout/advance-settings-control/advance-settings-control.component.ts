...
export class AdvanceSettingsControlComponent implements OnInit {
  ...
  getAsettingsById() {
    this.api.getAdvanceSettingsPermissions(this.id).subscribe(
      (res: any) => {
        //console.log(res,"res")
  ...
  edit() {
    this.api
      .updateAdvanceSettingsPermissions(this.id, this.allPermissions)
      .subscribe(
        (resp: any) => {
  ...
  updateChildrenMenuPermission(event: any, index: any, index2: any, data: any) {
    ...
    if (event.target.checked === true) {
      ...
      const statuscheckVal: boolean = this.allPermissions[
        index
      ].children_status.some((ele: any) => ele.access_status === true);
      ...
      const statucheckVal: boolean = this.allPermissions[
        index
      ].children_status.some((ele: any) => ele.access_status === true);
  ...
  updateParentMenuPermission(event: any, index: any, data: any) {
    ...
    if (event.target.checked === true) {
      ...
      data[index].access_status = false;
      data[index].children_status.forEach((ele: any) => {
        ele.access_status = false;
      });
    }
